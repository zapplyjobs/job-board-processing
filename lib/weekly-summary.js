...
const CONSUMER_REPOS = [
  { repo: 'New-Grad-Jobs-2027',                       label: 'New-Grad' },
  { repo: 'Internships-2027',                         label: 'Internships' },
  { repo: 'New-Grad-Software-Engineering-Jobs-2027',  label: 'Software-Eng' },
  { repo: 'New-Grad-Data-Science-Jobs-2027',          label: 'Data-Science' },
  { repo: 'New-Grad-Hardware-Engineering-Jobs-2027',  label: 'Hardware-Eng' },
  { repo: 'New-Grad-Healthcare-Jobs-2027',              label: 'Healthcare' },
];
...
function supabaseGet(key) {
  const supabaseUrl = process.env.SUPABASE_URL;
  const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!supabaseUrl || !supabaseKey) return Promise.resolve(null);
  return new Promise((resolve, reject) => {
    https.get(`${supabaseUrl}/rest/v1/pipeline_metrics?select=data&key=eq.${key}`, {
      headers: {
        'apikey': supabaseKey,
      ...
      res.on('end', () => {
        try {
          ...
          resolve(rows && rows[0] ? rows[0].data : null);
...
function weekRange(end = new Date()) {
  const endDate = new Date(end);
  const start = new Date(endDate);
  start.setDate(start.getDate() - 6);
  const fmt = d => d.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
  return { start, end: endDate, label: `${fmt(start)} – ${fmt(endDate)}` };
}

function parseArgs(argv) {
  const args = { mode: 'current', replayIndex: 0, noPost: false };
  for (let i = 0; i < argv.length; i++) {
    const arg = argv[i];
    if (arg === '--mode') {
      args.mode = argv[++i] || '';
    } else if (arg === '--replay-index') {
      args.replayIndex = Number(argv[++i] || '0');
    } else if (arg === '--no-post') {
      args.noPost = true;
    } else {
  ...
  if (!['current', 'replay'].includes(args.mode)) throw new Error(`Invalid --mode: ${args.mode}`);
  if (!Number.isInteger(args.replayIndex) || args.replayIndex < 0) throw new Error(`Invalid --replay-index: ${args.replayIndex}`);
  return args;
}

async function main() {
  const args = parseArgs(process.argv.slice(2));
  if (!CHANNEL_ID && !args.noPost) { console.error('DISCORD_WEEKLY_STATS_CHANNEL_ID not set'); process.exit(1); }

  let prevStats = {};
  if (fs.existsSync(STATS_FILE)) {
    try { prevStats = JSON.parse(fs.readFileSync(STATS_FILE, 'utf8')); } catch {}
  }
  const snapshots = Array.isArray(prevStats.snapshots) ? prevStats.snapshots : [];
  if (args.mode === 'replay') {
    const snapshot = snapshots[args.replayIndex];
    if (!snapshot) {
      throw new Error(`No stored weekly snapshot at replay index ${args.replayIndex}. Current file has ${snapshots.length} snapshot(s). Historical replay is only available for previously captured weeks.`);
    }
    if (!args.noPost) {
      ...
      for (const msg of snapshot.messages) await channel.send(msg);
      await client.destroy();
    }
    console.log(`✅ Replayed stored weekly snapshot: ${snapshot.week_range} (index ${args.replayIndex})`);
    return;
  }
  ...
  // Query Supabase for pool total + source breakdown + enriched count (migration-proof, no R2 dependency)
  const [zjpMetrics, jobsMetadata, enrichStats] = await Promise.all([
    supabaseGet('zjp-metrics'),
    supabaseGet('jobs-metadata'),
    supabaseGet('enrichment-stats'),
  ]);

  if (zjpMetrics && zjpMetrics.pool) {
    pipelineTotal = zjpMetrics.pool.total_jobs;
  ...
  // By-source breakdown from jobs-metadata (Supabase)
  if (jobsMetadata && jobsMetadata.by_source) {
    const parts = ['workday','greenhouse','amazon','ashby','lever','oracle','greenhouse']
      .filter(k => jobsMetadata.by_source[k])
      .map(k => `${k[0].toUpperCase()}${k.slice(1)}: ${fmtNum(jobsMetadata.by_source[k])}`);
    if (parts.length) pipelineLines += `${'By source'.padEnd(28)} ${parts.join(' | ')}\n`;
  }

  // Enriched jobs count from enrichment-stats (Supabase)
  if (enrichStats && enrichStats.total_enriched != null) {
    pipelineLines += `${'Enriched jobs'.padEnd(28)} ${fmtNum(enrichStats.total_enriched)}\n`;
  }
  ...
  const consumerResults = await Promise.all(CONSUMER_REPOS.map(async ({ repo, label }) => {
    ...
    try {
      const [latestCommits, raw] = await Promise.all([
        githubGet(`/repos/${ORG}/${repo}/commits?per_page=1`),
        rawGet(`https://raw.githubusercontent.com/${ORG}/${repo}/main/${filePath}?t=${Date.now()}`)
      ]);
      const latest = latestCommits[0] || null;
      const ts = latest?.commit?.committer?.date || latest?.commit?.author?.date;
      const ageStr = ts ? (() => {
  ...
  const issues = [];
  for (const [name, w] of Object.entries(byWorkflow)) {
    if (w.fail > 0) issues.push(`${w.fail} failed run(s) in ${name}`);
  }
  if (history.length >= 3 && pipelineTotal && pipelineTotal < (history.reduce((s, v) => s + v, 0) / history.length) * 0.85) {
    issues.push('job count drop');
  }
  const verdict = issues.length === 0 ? '✅ All systems healthy' : `⚠️ ${issues.length} issue(s): ${issues.slice(0, 3).join(', ')}`;

  // --- Build messages ---
  const msg1 = `${verdict}\n\n📊 **zapplyjobs Org — Weekly Summary**\nWeek of ${week.label}\n\n⭐ **REPOSITORY STATS**\n\`\`\`\n${header}\n${divider}\n${repoLines}\`\`\`${totalsLine}`;
  ...
  const newStats = { date: new Date().toISOString(), stars: {}, forks: {}, pipelineTotal, history: newHistory, snapshots: [snapshot, ...snapshots].slice(0, SNAPSHOT_KEEP) };
  for (const name of TRACKED_REPOS) {
    if (repoMap[name]) {
