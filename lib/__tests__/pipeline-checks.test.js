...
function requireCheckModule(path) {
  try {
    return require(`../jobs-data-scripts/${path}`);
  } catch {
    return require(`../${path}`);
  }
}
const config = requireCheckModule('checks/config');
...
const check04 = requireCheckModule('checks/check-04-job-drop');
...
// --- Check 5: Source drop ---
const check05 = requireCheckModule('checks/check-05-source-drop');
...
const check09 = requireCheckModule('checks/check-09-domain-empty');
...
const check10 = requireCheckModule('checks/check-10-senior-filter');
...
// --- Check 11: G1 rate ---
const check11 = requireCheckModule('checks/check-11-g1-rate');
...
const check12 = requireCheckModule('checks/check-12-enrich-coverage');
...
const check21 = requireCheckModule('checks/check-21-dedupe-size');
...
const check22 = requireCheckModule('checks/check-22-metadata-completeness');
...
const check25 = requireCheckModule('checks/check-25-r2-freshness');
...
test('all checks export { id, name, check }', () => {
  ...
  for (const name of checkFiles) {
    const mod = requireCheckModule(`checks/${name}`);
...
// --- Check 17 repo map stays on canonical 2027 repos ---
test('check-17: uses canonical 2027 consumer repos', () => {
  const fs = require('fs');
  const path = require('path');
  const file = path.join(__dirname, '..', 'checks', 'check-17-bump-failed.js');
  ...
  for (const repo of expectedRepos) {
    assert.ok(source.includes(repo), `missing canonical repo ${repo}`);
  }
  assert.ok(!source.includes('zapplyjobs/New-Grad-Jobs-2026'), 'should not reference retired 2026 consumer repos');
  assert.ok(!source.includes('zapplyjobs/Internships-2026'), 'should not reference retired 2026 consumer repos');
  assert.ok(!source.includes('zapplyjobs/New-Grad-Software-Engineering-Jobs-2026'), 'should not reference retired 2026 consumer repos');
  assert.ok(!source.includes('zapplyjobs/New-Grad-Data-Science-Jobs-2026'), 'should not reference retired 2026 consumer repos');
  assert.ok(!source.includes('zapplyjobs/New-Grad-Hardware-Engineering-Jobs-2026'), 'should not reference retired 2026 consumer repos');
  assert.ok(!source.includes('zapplyjobs/New-Grad-Healthcare-Jobs-2026'), 'should not reference retired 2026 consumer repos');
});
