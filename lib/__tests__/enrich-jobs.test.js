...
test('bounded Workday LCA aliases resolve exact legal entities', () => {
...
test('balances stale reserve so no-skills does not starve no-visa fixes', () => {
  ...
  const latest = new Map([
    ...
    ['fresh-1', { version: 85, hasDescription: true, hasSkills: true, hasVisa: true }],
  ]);
  const { fastBatch, staleReserved, noSkillsReserved, noVisaReserved } = buildFastBatch(fastPending, latest, 85, 5, 3);
...
test('rescues current-version exhausted record when description now exists', () => {
  ...
  const processed = { status: 'exhausted', enricher_version: 85 };
...
test('does not rescue non-exhausted record', () => {
  const descriptions = new Map([['microsoft-123', 'Full description now available']]);
  const job = { id: 'microsoft-123', source: 'microsoft', description: null };
  const processed = { status: 'retry', enricher_version: 85 };
  const latest = { id: 'microsoft-123', has_description: false };
  assert.strictEqual(shouldRescueExhaustedRecord(job, descriptions, processed, latest), false);
});
