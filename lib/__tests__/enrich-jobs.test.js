...
const {
  ...
  toPlainText, splitSections, matchSkills, detectVisa,
  extractMinDegree, inferDegreeFromTitle, extractExperienceLevel,
  buildWdDescUrl, buildSrDescUrl, calculateAdaptiveStaleReserve, buildFastBatch, buildVisaDetectionText, ENRICHER_VERSION,
...
const { loadTaxonomy } = require('../enrich/taxonomy');
...
test('bounded Workday and Oracle LCA aliases resolve exact legal entities', () => {
  const lca = makeLcaSet([
    'astreya partners',
  ...
  assert.strictEqual(isPossibleSponsor('Astreya', lca), true);
  assert.strictEqual(isPossibleSponsor('Becton Dickinson', lca), true);
  assert.strictEqual(isPossibleSponsor('Cummins', lca), true);
  assert.strictEqual(isPossibleSponsor('Live Nation', lca), true);
  assert.strictEqual(isPossibleSponsor('Nokia', lca), true);
  assert.strictEqual(isPossibleSponsor('Symbotic', lca), true);
});
...
test('ENR-SKILLS-2 taxonomy extracts Workday facilities and industrial controls terms', () => {
  const termMap = loadTaxonomy();
  const jll = matchSkills('Maintain building automation systems across AHUs, VAV boxes, boilers, chillers, HVAC and BAS systems using Smart Building Analytics.', termMap);
  assert.ok(jll.includes('building automation systems'));
  assert.ok(jll.includes('hvac'));
  assert.ok(jll.includes('bas'));
  assert.ok(jll.includes('ahus'));
  assert.ok(jll.includes('vav'));
  assert.ok(jll.includes('boilers'));
  assert.ok(jll.includes('chillers'));
  assert.ok(jll.includes('smart building analytics'));

  const kion = matchSkills('Perform conveyor system layout and conveyor installation for warehouse automation equipment.', termMap);
  assert.ok(kion.includes('conveyor system'));
  assert.ok(kion.includes('conveyor installation'));
});

test('ENR-SKILLS-2 taxonomy extracts defense electronics terms but avoids parked sales and biotech prose', () => {
  const termMap = loadTaxonomy();
  const amentum = matchSkills('Submarine Electronic Warfare technician supporting AN/BLQ-10 and BLQ-10 systems.', termMap);
  assert.ok(amentum.includes('electronic warfare'));
  assert.ok(amentum.includes('BLQ-10'));

  const verizon = matchSkills('Technical sales experience with partner onboarding, pricing, packaging, and go-to-market execution.', termMap);
  assert.deepStrictEqual(verizon, []);

  const astraZeneca = matchSkills('Microbiology, immunology, ELISA, western blot, flow staining, mouse models, and bacterial manipulation.', termMap);
  assert.deepStrictEqual(astraZeneca, []);
});

test('ENR-D4 taxonomy extracts electrical/power/safety-engineering terms (v89)', () => {
  const termMap = loadTaxonomy();
  // Vertiv electrical systems engineer — 3-phase power distribution products
  const vertiv = matchSkills('Design and validation of Static Transfer Switch (STS) and Power Distribution Unit (PDU) 3-phase power products. Deep knowledge of electrical power distribution systems.', termMap);
  assert.ok(vertiv.includes('3-phase'), 'should extract 3-phase');
  assert.ok(vertiv.includes('power distribution'), 'should extract power distribution');

  // Product safety engineer — IEC safety standards
  const safety = matchSkills('Working knowledge within IEC 62368-1 & IEC 61010-1 and some of its collateral.', termMap);
  assert.ok(safety.includes('iec 61010'), 'should extract iec 61010');
  assert.ok(safety.includes('iec 62368'), 'should extract iec 62368');

  // No false positive: generic operations/finance prose must not match these engineering terms
  const ops = matchSkills('Manage fleet operations, customer returns, and shift scheduling for automotive service locations.', termMap);
  assert.ok(!ops.includes('3-phase'));
  assert.ok(!ops.includes('power distribution'));
});
...
test('negative signal: legal authorization without sponsorship now or later', () => {
  const text = 'Legal authorization to work in the United States without sponsorship now or in the future.';
  assert.strictEqual(detectVisa(text), false);
});

test('negative signal: temporary visa holders not eligible for hire', () => {
  const text = 'Individuals with temporary visas such as E, F-1, H-1, H-2, L, B, J or TN, or who need sponsorship for work authorization now or in the future, are not eligible for hire.';
  assert.strictEqual(detectVisa(text), false);
});

test('Oracle page description supplements cached text for visa detection', () => {
  const cached = 'Role summary without posting-level work authorization language.';
  const oraclePage = 'Legal authorization to work in the United States without sponsorship now or in the future.';
  assert.strictEqual(detectVisa(cached), null);
  assert.strictEqual(detectVisa(buildVisaDetectionText(cached, oraclePage)), false);
});
...
test('negative signal: U.S. Person status required for ITAR compliance', () => {
  const text = 'Ability to work independently and adapt to changing priorities – required U.S. Person status for compliance with ITAR regulations – required.';
  assert.strictEqual(detectVisa(text), false);
});

test('negative signal: successful applicants must be a US Person', () => {
  const text = 'Due to the type of work at the facility and certain access restrictions, successful applicants must be a “US Person” (US Citizens, US Nationals, lawful permanent residents, asylees or refugees).';
  assert.strictEqual(detectVisa(text), false);
});

test('negative signal: US Citizen / Perm Resident / Other US Person', () => {
  const text = 'US Citizen/Perm Resident/ Other US Person.';
  assert.strictEqual(detectVisa(text), false);
});

test('negative signal: U.S. Person status for ITAR-controlled technical data', () => {
  const text = 'This position requires U.S. Person status for access to ITAR-controlled technical data.';
  assert.strictEqual(detectVisa(text), false);
});

test('negative signal: able to obtain and maintain a secret security clearance', () => {
  const text = 'Candidate is able to obtain and maintain a SECRET security clearance.';
  assert.strictEqual(detectVisa(text), false);
});

test('negative signal: US citizenship with ability to maintain government clearance', () => {
  const text = 'US Citizenship. with ability to attain/maintain government security clearance.';
  assert.strictEqual(detectVisa(text), false);
});

test('negative signal: ability to obtain U.S. Government security clearance', () => {
  const text = 'Active or ability to obtain a U.S. Government security clearance.';
  assert.strictEqual(detectVisa(text), false);
});

test('negative signal: candidate must be a U.S. Person (VISA-10)', () => {
  const text = 'Candidate must be a U.S. Person (U.S. citizen, U.S. permanent resident, asylee or refugee).';
  assert.strictEqual(detectVisa(text), false);
});

test('negative signal: Q-level clearance requires U.S. citizenship (VISA-10)', () => {
  const text = 'Ability to secure and maintain a U.S. DOE Q-level security clearance which requires U.S. citizenship.';
  assert.strictEqual(detectVisa(text), false);
});

test('negative signal: noun-form authorization to work without sponsorship (VISA-10)', () => {
  const text = 'Authorization to work in the United States without sponsorship.';
  assert.strictEqual(detectVisa(text), false);
});
...
test('myworkdaysite URL → API URL with tenant map (ENR-WORKDAY-SLA-1)', () => {
  // Snap's career slug 'snap' != WD internal tenant 'snapchat'. Without the map, API returns 422.
  ...
  assert.strictEqual(result, 'https://wd1.myworkdaysite.com/wday/cxs/snapchat/snap/job/New-York-New-York/Software-Engineer--Backend--Level-5_R0045614');
});

test('myworkdaysite unknown tenant falls back to slug (will 422, failCache handles)', () => {
  const result = buildWdDescUrl('https://wd1.myworkdaysite.com/unknown-co/job/Location/Title_R1');
  assert.strictEqual(result, 'https://wd1.myworkdaysite.com/wday/cxs/unknown-co/unknown-co/job/Location/Title_R1');
});
...
test('reserves a small slice for stale fast records', () => {
  ...
  const latest = new Map([
    ['stale-1', { version: 67, hasDescription: true, hasSkills: true }],
    ['stale-2', { version: 68, hasDescription: true, hasSkills: true }],
    ['fresh-1', { version: 69, hasDescription: true, hasSkills: true }],
    ['fresh-2', { version: 69, hasDescription: true, hasSkills: true }],
  ]);
...
test('prefers no-description stale records inside reserved slice', () => {
  ...
  const latest = new Map([
    ['fresh-1', { version: 71, hasDescription: true, hasSkills: true }],
    ['fresh-2', { version: 71, hasDescription: true, hasSkills: true }],
    ['stale-desc', { version: 69, hasDescription: true, hasSkills: true }],
    ['stale-nodesc', { version: 69, hasDescription: false, hasSkills: false }],
  ]);
...
test('prefers no-skills stale records after no-description inside reserved slice', () => {
  const fastPending = [
    { id: 'fresh-1', posted_at: '2026-05-30T10:00:00Z' },
    { id: 'stale-has-skills', posted_at: '2026-05-29T10:00:00Z' },
    { id: 'stale-no-skills', posted_at: '2026-05-29T09:00:00Z' },
    { id: 'fresh-2', posted_at: '2026-05-30T09:00:00Z' },
  ];
  const latest = new Map([
    ['fresh-1', { version: 71, hasDescription: true, hasSkills: true, hasVisa: true }],
    ['fresh-2', { version: 71, hasDescription: true, hasSkills: true, hasVisa: true }],
    ['stale-has-skills', { version: 69, hasDescription: true, hasSkills: true, hasVisa: true }],
    ['stale-no-skills', { version: 69, hasDescription: true, hasSkills: false, hasVisa: true }],
  ]);
  const { fastBatch, staleReserved, noSkillsReserved } = buildFastBatch(fastPending, latest, 71, 3, 1);
  assert.deepStrictEqual(fastBatch.map(j => j.id), ['stale-no-skills', 'fresh-1', 'fresh-2']);
  assert.strictEqual(staleReserved, 1);
  assert.strictEqual(noSkillsReserved, 1);
});
...
test('prefers no-visa stale records after no-description inside reserved slice', () => {
  ...
  const latest = new Map([
    ['fresh-1', { version: 71, hasDescription: true, hasSkills: true, hasVisa: true }],
    ['fresh-2', { version: 71, hasDescription: true, hasSkills: true, hasVisa: true }],
    ['stale-has-visa', { version: 69, hasDescription: true, hasSkills: true, hasVisa: true }],
    ['stale-no-visa', { version: 69, hasDescription: true, hasSkills: true, hasVisa: false }],
  ]);
...
test('balances stale reserve so no-skills does not starve no-visa fixes', () => {
  const fastPending = [
    { id: 'no-skills-1' },
    { id: 'no-skills-2' },
    { id: 'no-skills-3' },
  ...
  const latest = new Map([
    ['no-skills-1', { version: 84, hasDescription: true, hasSkills: false, hasVisa: false }],
    ['no-skills-2', { version: 84, hasDescription: true, hasSkills: false, hasVisa: false }],
    ['no-skills-3', { version: 84, hasDescription: true, hasSkills: false, hasVisa: false }],
    ['no-visa-1', { version: 84, hasDescription: true, hasSkills: true, hasVisa: false }],
    ['no-visa-2', { version: 84, hasDescription: true, hasSkills: true, hasVisa: false }],
    ['fresh-1', { version: 86, hasDescription: true, hasSkills: true, hasVisa: true }],
  ]);
  const { fastBatch, staleReserved, noSkillsReserved, noVisaReserved } = buildFastBatch(fastPending, latest, 86, 5, 3);
  assert.deepStrictEqual(fastBatch.map(j => j.id), ['no-skills-1', 'no-visa-1', 'no-skills-2', 'fresh-1']);
  assert.strictEqual(staleReserved, 3);
  assert.strictEqual(noSkillsReserved, 2);
  assert.strictEqual(noVisaReserved, 1);
});
test('adaptive stale reserve expands when stale version rows dominate', () => {
  assert.strictEqual(calculateAdaptiveStaleReserve(5443, 446, 500, 100, 100), 400);
});

test('adaptive stale reserve keeps a fresh-job floor when fresh rows exist', () => {
  assert.strictEqual(calculateAdaptiveStaleReserve(800, 800, 500, 100, 100), 400);
});

test('adaptive stale reserve releases full capacity when no fresh rows remain', () => {
  assert.strictEqual(calculateAdaptiveStaleReserve(600, 0, 500, 100, 100), 500);
});

test('default fast batch uses adaptive stale reserve and protects fresh jobs', () => {
  const fastPending = [
    { id: 'stale-1' },
    { id: 'stale-2' },
    { id: 'stale-3' },
  ...
  const latest = new Map();
  for (let i = 1; i <= 8; i++) {
    latest.set(`stale-${i}`, { version: 84, hasDescription: true, hasSkills: true, hasVisa: true });
  }
  for (let i = 1; i <= 3; i++) {
    latest.set(`fresh-${i}`, { version: 86, hasDescription: true, hasSkills: true, hasVisa: true });
  }
  const { fastBatch, staleReserved, staleReserveLimit } = buildFastBatch(fastPending, latest, 86, 10);
  assert.strictEqual(staleReserveLimit, 8);
  assert.strictEqual(staleReserved, 8);
  assert.deepStrictEqual(fastBatch.map(j => j.id), ['stale-1', 'stale-2', 'stale-3', 'stale-4', 'stale-5', 'stale-6', 'stale-7', 'stale-8', 'fresh-1', 'fresh-2']);
});
