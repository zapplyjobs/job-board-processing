    ...
    for (const company of allCompanies) {
      ...
      if (yield_ > 0) {
        ...
        if (newStreak >= threshold && !knownZeroYield.has(company)) {
          // AGG-FETCH-14: Prefer fetcher_health from metadata over HTTP probing.
          // fetcher_health is written by fetchers during the pipeline run — no network calls needed.
          if (!prevReason) {
            const healthEntry = (ctx.metadata.fetcher_health || {})[company];
            if (healthEntry) {
              // Map fetcher health status to check-19 classification
              // 'alive' = has jobs (shouldn't be here, but classify as dormant)
              // 'zero' = API returned 200 with 0 results → dormant
              // 'error' = API returned error → dead_slug
              newState[company].reason = healthEntry.status === 'error' ? 'dead_slug' :
                                         healthEntry.jobs > 0 ? 'alive' : 'dormant';
            } else {
              // Fallback: no health data for this company — probe ATS directly
