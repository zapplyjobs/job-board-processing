...
// Sources where low retrievable_description_pct is structural (short platform descriptions, not regression).
// ENR-ORACLE-RETRIEVABLE-DISCREPANCY-1 (2026-07-19): oracle median desc length = 131 chars (very short);
// 75% retrievable is chronic baseline (C186 noted "oracle T3+=42% = content absence"). Without this exemption,
// check-32 fires forever on oracle creating alert fatigue. To remove a source here, justify that its
// retrievable rate has structurally improved above 90% floor.
const STRUCTURALLY_LOW_RETRIEVABLE = new Set(['oracle']);
