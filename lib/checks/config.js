...
// Per-source retrievable floor overrides (ENR-CHRONIC-ALARM-CLASS-1, 2026-07-19).
// Sources with known chronic-below-90% baselines get a lower floor so check-32
// only fires on REAL regression (drop below chronic baseline), not on the chronic
// condition itself. Replaces STRUCTURALLY_LOW_RETRIEVABLE (binary exempt) with a
// graduated approach that preserves regression detection.
// ENR-ORACLE-RETRIEVABLE-DISCREPANCY-1: oracle median desc len = 131 chars → ~62%
// retrievable is chronic baseline. Floor set to 0.50 — alarm only fires if oracle
// drops below 50% (real regression). Adjust if oracle's chronic baseline changes.
const PER_SOURCE_RETRIEVABLE_FLOOR_OVERRIDES = { oracle: 0.50 };
...
module.exports = {
  ...
  PER_SOURCE_RETRIEVABLE_FLOOR_OVERRIDES,
  NO_SIDECAR_SOURCES,
  KNOWN_ZERO_YIELD,
  ...
  thresholds: {
    ...
    sidecarIntegrityMinEntries: 100,  // ENR-SIDECAR-INTEGRITY-CHECK-1 (check-33): min sidecar entries to alert
    sidecarIntegrityMaxEmptyPct: 5,   // ENR-SIDECAR-INTEGRITY-CHECK-1 (check-33): max % entries with empty description_text
  },
