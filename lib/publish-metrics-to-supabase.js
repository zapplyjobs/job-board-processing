...
function summarizePostedJobs(data) {
  ...
  // OUT-CHANNEL-NAMES-1 fix: build channelId → channelName map from posts that HAVE
  // channelName persisted (new posts since the channelName persistence shipped).
  // Apply this map to ALL posts so old posts (without channelName) also aggregate
  // by name. This merges the split (old ID-keyed + new name-keyed) into a single
  // name-keyed entry per channel. Map grows as more posts accumulate channelName.
  ...
  for (const j of jobs) {
    ...
    // OUT-CHANNEL-NAMES-1: resolve to channelName via the dynamic map.
    // Lookup chain: j.channel/name (legacy v1) → discordPosts[firstId].channelName (new posts) →
    // channelNameMap[firstId] (old posts resolved via map built from new posts) → fallback to raw channelId.
    ...
    if (ts) {
      ...
      const ageDays = (now - new Date(ts).getTime()) / dayMs;
