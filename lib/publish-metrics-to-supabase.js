...
function summarizePostedJobs(data) {
  ...
  for (const j of jobs) {
    ...
    // OUT-CHANNEL-NAMES-1: prefer human-readable channelName over snowflake ID.
    // Lookup chain: top-level j.channel/name (legacy v1) → discordPosts[firstId].channelName (v2 producer-persisted) → fallback to channelId.
