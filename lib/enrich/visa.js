...
function withoutLegalSuffix(name) {
  return name.replace(/\b(llc|inc|corp|corporation|company|co|ltd|limited)\b/g, '').replace(/\s+/g, ' ').trim();
}
...
function isPossibleSponsor(companyName, lcaSet) {
  ...
  if (alias) {
    const normalizedAlias = normalizeLcaName(alias);
    if (lcaSet.has(normalizedAlias) || lcaSet.has(withoutLegalSuffix(normalizedAlias))) return true;
  }
  return null;
}
...
const VISA_NEGATIVE = [
  ...
  /legal authori[sz]ation to work in (?:the )?(?:u\.?s\.?|united states)[^.]{0,160}without sponsorship/i,
  /individuals with temporary visas[^.]{0,180}need sponsorship[^.]{0,80}not eligible for hire/i,
  ...
  /candidate must be (?:a|an)?\s*["“”"]?\s*u\.?s\.?\s*person["“”"]?.{0,120}(?:citizens?|green card|permanent resident|refugee|asylee)/i,
  ...
  /citizenship is required for all positions with a government clearance/i,
  /citizenship due to contract requirements/i,
  /\bcitizenship required\b/i,
  /\bcitizenship is required to obtain a security clearance\b/i,
  /(?:q[- ]?level|doe q).{0,80}clearance.{0,80}(?:u\.?s\.? citizenship|citizenship)/i,
  /does not sponsor individuals for employment-based visas/i,
  /not open for visa sponsorship/i,
  /citizenship status is required/i,
  /(citizen|u\.?s\.? national|legal permanent resident|asylee|refugee).{0,140}eligible to apply for and obtain the appropriate export control license/i,
  /unable to consider candidates who require visa sponsorship/i,

  /not eligible for (?:visa )?sponsorship/i,
  /not eligible for (?:an?\s+)?(?:[a-z0-9&.-]+\s+){0,3}immigration sponsorship/i,
  /does not sponsor\/?support[^.]{0,120}(h-?1b|tn|forms? i-?983|stem opt)/i,
  /work authorization[^.]{0,160}does not require sponsorship(?:[^.]{0,40}for a visa)?/i,
  /currently authorized to work[^.]{0,160}must not require[^.]{0,120}sponsorship/i,
  /authori[sz]ation to work in (?:the )?(?:u\.?s\.?|united states)[^.]{0,120}without (?:restriction or )?sponsorship/i,
  /only\s+u\.?s\.?\s+persons?\s+will\s+be\s+considered/i,
  /only\s+u\.?s\.?\s+(?:citizens?|naturalized citizens?|permanent residents?)[^.]{0,180}will be considered/i,
  /not considering applicants that need any type of immigration sponsorship/i,
  /ability to obtain and maintain(?: a| an)?(?: minimum of a)? [^.]{0,80}security clearance/i,
  /must be able to obtain and maintain(?: a| an)? [^.]{0,80}security clearance/i,
  /ability to obtain and maintain a public trust security clearance/i,
  /applicants? must be u\.?s\.? persons? as defined by (?:itar|ear)/i,
  /u\.?s\.?\s*person status.{0,120}(?:itar|export control).{0,60}required/i,
  /successful applicants? must be (?:a|an)?\s*[\"“”']?u\.?s\.?\s*person[\"“”']?.{0,160}(?:citizens?|nationals?|permanent residents?|asylees?|refugees?)/i,
  /u\.?s\.?\s*citizenship.{0,80}ability to (?:attain|obtain|maintain|attain\/maintain).{0,80}(?:government )?security clearance/i,
  /u\.?s\.?\s*citizen\s*\/\s*perm(?:anent)?\s*resident\s*\/\s*other\s*u\.?s\.?\s*person/i,
  /u\.?s\.?\s*person status.{0,80}(?:itar|export control)-?controlled technical data/i,
  /able to obtain and maintain(?: a| an)? [^.]{0,80}(?:secret|top secret|government)? security clearance/i,
  /ability to obtain(?: a| an)? u\.?s\.? government security clearance/i,
];
...
function extractMicrosoftPageDescription(html) {
  if (!html) return null;
  const m = html.match(/<meta name=\"description\" content=\"([\s\S]*?)\"(?:\s*\/?)><meta/i);
  if (!m) return null;
...
function buildOracleDetailsUrl(job) {
  const pageUrl = job.apply_url || job.url;
  if (!pageUrl) return null;
  const m = pageUrl.match(/^https:\/\/([^/]+)\/hcmUI\/CandidateExperience\/en\/sites\/([^/]+)\/job\/([^/?#]+)/i);
  if (!m) return null;
  const [, host, siteNumber, jobId] = m;
  return `https://${host}/hcmRestApi/resources/latest/recruitingCEJobRequisitionDetails?expand=all&onlyData=true&finder=ById;Id=%22${jobId}%22,siteNumber=${siteNumber}`;
}

function extractOraclePageDescription(body) {
  if (!body) return null;
  let parsed;
  try {
  ...
  ].filter(Boolean).map(stripHtmlToText).filter(Boolean);
...
async function fetchApplicationVisaStatus(job) {
  try {
    ...
    if (job.source === 'microsoft') {
      const pageUrl = job.apply_url || job.url;
      if (!pageUrl) return { visaPresent: null, questionCount: null, pageDescription: null };
      const result = await httpsGet(pageUrl);
      if (!result || result.status !== 200) return { visaPresent: null, questionCount: null, pageDescription: null };
      return {
        visaPresent: extractMicrosoftVisaQuestionPresence(result.body),
        questionCount: null,
        pageDescription: extractMicrosoftPageDescription(result.body),
      };
    }

    if (job.source === 'oracle') {
      const detailsUrl = buildOracleDetailsUrl(job);
      if (!detailsUrl) return { visaPresent: null, questionCount: null, pageDescription: null };
      const result = await httpsGet(detailsUrl);
      if (!result || result.status !== 200) return { visaPresent: null, questionCount: null, pageDescription: null };
      return { visaPresent: null, questionCount: null, pageDescription: extractOraclePageDescription(result.body) };
    }



    return { visaPresent: null, questionCount: null, pageDescription: null };
  } catch (_) {
    return { visaPresent: null, questionCount: null, pageDescription: null };
  }
}
...
module.exports = {
  ...
  extractMicrosoftPageDescription,
  buildOracleDetailsUrl,
  extractOraclePageDescription,
