...
const STRUCTURAL_T0_SOURCES = new Set(['simplify', 'eightfold', 'jsearch']);
const TAG_CONTENT_SOURCES = new Set(['workday', 'smartrecruiters']);
...
function classifyResidualOwner(source, sourceStats, tiers) {
  const residual = (tiers.t0 || 0) + (tiers.t1 || 0) + (tiers.t2 || 0);
  if (residual === 0) return { owner: 'healthy', reason: 'none', note: 'No below-T3 residual' };
  if (STRUCTURAL_T0_SOURCES.has(source)) {
    return { owner: 'structural', reason: 'no_description_source', note: 'Fallback source without reliable description path' };
  }
  if (source === 'microsoft') {
    return { owner: 'agg_source', reason: 'description_regression', note: 'Source-side descriptions missing or regressed' };
  }
  if (source === 'oracle') {
    return { owner: 'agg_source', reason: 'detail_or_summary_quality', note: 'Detail fetch/text quality upstream' };
  }
  if (TAG_CONTENT_SOURCES.has(source)) {
    return { owner: 'tag_content', reason: 'content_or_pool_quality', note: 'Residual mostly TAG/content-side' };
  }
  const descPct = sourceStats.tech_us > 0 ? sourceStats.has_desc / sourceStats.tech_us : 0;
  if ((tiers.t0 || 0) > 0 && descPct < 0.5) {
    return { owner: 'agg_source', reason: 'low_description_coverage', note: 'Too many tech-US jobs still lack descriptions' };
  }
  return { owner: 'mixed', reason: 'needs_sampling', note: 'Residual needs fresh sampling before ownership is assigned' };
}

function generateStats(ctx) {
  const { allJobs, finalLines, descriptionsMap, DATA_DIR, ENRICHER_VERSION, descWaiting, reenrichmentPending, rescuedExhausted = 0 } = ctx;
  ...
  const residualByOwner = { structural: 0, agg_source: 0, tag_content: 0, enr_owned_signal: 0, mixed: 0 };
  const sourceExplanations = [];
  for (const [src, tiers] of Object.entries(tiersBySource)) {
    const sourceStats = statsBySource[src] || { tech_us: 0, has_desc: 0, enriched: 0 };
    const residualCount = (tiers.t0 || 0) + (tiers.t1 || 0) + (tiers.t2 || 0);
    const ownership = classifyResidualOwner(src, sourceStats, tiers);
    if (ownership.owner === 'structural') residualByOwner.structural += residualCount;
    else if (ownership.owner === 'agg_source') residualByOwner.agg_source += residualCount;
    else if (ownership.owner === 'tag_content') residualByOwner.tag_content += residualCount;
    else if (ownership.owner === 'mixed') residualByOwner.mixed += residualCount;
    sourceExplanations.push({
      source: src,
      residual_count: residualCount,
      residual_pct: sourceStats.tech_us > 0 ? Math.round(100 * residualCount / sourceStats.tech_us) : 0,
  ...
  const visaReasonBySource = {};
  for (const line of finalLines) {
    try {
      const obj = JSON.parse(line);
      if (!techUsIds.has(obj.id)) continue;
      const reason = obj.visa_no_signal_reason;
      if (reason === null || reason === undefined) continue;
      visaReasonCounts[reason] = (visaReasonCounts[reason] || 0) + 1;
      const src = obj.source || 'unknown';
      if (!visaReasonBySource[src]) visaReasonBySource[src] = {};
      visaReasonBySource[src][reason] = (visaReasonBySource[src][reason] || 0) + 1;
    } catch (_) {}
  }
  ...
  residualByOwner.enr_owned_signal = rescuedExhausted + stuckRecords.length;
  ...
  const enrichmentStats = {
    ...
    by_source: Object.fromEntries(
      Object.entries(statsBySource).map(([src, v]) => {
        ...
        const tiers = tiersBySource[src] || { t0: 0, t1: 0, t2: 0, t3: 0, t4: 0 };
        const ownership = classifyResidualOwner(src, v, tiers);
        const residualCount = (tiers.t0 || 0) + (tiers.t1 || 0) + (tiers.t2 || 0);
        return [src, { ...v,
          ...
          residual_count: residualCount,
          residual_pct: v.tech_us > 0 ? Math.round(100 * residualCount / v.tech_us) : 0,
          primary_gap_owner: ownership.owner,
          primary_gap_reason: ownership.reason,
          primary_gap_note: ownership.note,
        }];
      })
    ),
    ...
    residual_explanations: {
      by_owner: residualByOwner,
      by_source: sourceExplanations.sort((a, b) => b.residual_count - a.residual_count),
      current_run: {
        late_description_rescues: rescuedExhausted,
        stuck_records: stuckRecords.length,
        desc_waiting: descWaiting,
        reenrichment_pending: reenrichmentPending,
      },
      visa_no_signal_reasons: visaReasonCounts,
      visa_no_signal_by_source: visaReasonBySource,
    },
  };
...
module.exports = { generateStats, classifyTier, classifyResidualOwner, TECH_DOMAINS };
