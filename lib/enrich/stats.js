...
function generateStats(ctx) {
  ...
  // D3 (C181): honest description-quality metrics. has_description is a FLAG set at enrichment;
  // it lies when the text was never persisted (inline sources) or later removed. Retrievable text
  // (descriptionsMap truthy) is what a consumer can actually read. silent_rot = flagged but no text.
  const totalRetrievableDesc = finalLines.filter(o => descriptionsMap.get(o.id)).length;
  const silentRotCount = finalLines.filter(o => o.has_description && !descriptionsMap.get(o.id)).length;
