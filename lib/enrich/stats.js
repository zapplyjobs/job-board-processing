...
// ENR-QUALITY-8: hasRetrievableDesc is the honest signal (descriptionsMap has truthy text for the id).
// It defaults to the has_description flag for backward-compat, but the production loop passes the
// retrievable signal so tiers reflect READABLE text, not a point-in-time flag (the conflation that
// let "93% T3+" hide retrievable at 66%).
function classifyTier(obj, hasRetrievableDesc = !!obj.has_description) {
  const hasDesc = hasRetrievableDesc;
...
// ENR-QUALITY-8: Weighted Completeness Index (WCI) — a single 0-100 honest score per job, weighted by
// user-impact (description text most critical, then skills, then degree/visa). Uses the retrievable
// signal so a job missing readable text scores low regardless of the has_description flag. The single
// honest headline metric that would have prevented the "93% T3+ vs 66% retrievable" misread.
const WCI_WEIGHTS = { description: 0.40, skills: 0.30, degree: 0.15, visa: 0.15 };
function computeWCI(obj, hasRetrievableDesc) {
  const hasSkills = obj.required_skills?.length > 0;
  const hasDegree = obj.min_degree !== null && obj.min_degree !== undefined;
  const hasVisa = obj.sponsors_visa !== null || obj.visa_question_present !== null || obj.possible_sponsor !== null;
  const score = (hasRetrievableDesc ? WCI_WEIGHTS.description : 0)
    + (hasSkills ? WCI_WEIGHTS.skills : 0)
    + (hasDegree ? WCI_WEIGHTS.degree : 0)
    + (hasVisa ? WCI_WEIGHTS.visa : 0);
...
function generateStats(ctx) {
  ...
  // D3 (C181): honest description-quality metrics, counted in the loop below (tech-US scope,
  // matching totalEnriched). has_description is a point-in-time FLAG; retrievable = descriptionsMap
  // currently has truthy text for the id. silent_rot = flagged but no retrievable text.
  let totalRetrievableDesc = 0;
  let silentRotCount = 0;
  let totalWciSum = 0;
  for (const line of finalLines) {
    try {
      ...
      if (descriptionsMap.get(obj.id)) totalRetrievableDesc++;
      if (obj.has_description && !descriptionsMap.get(obj.id)) silentRotCount++;
      ...
      // Tier classification (ENR-QUALITY-8: use retrievable text, not the has_description flag) + WCI
      const hasRetrievableDesc = !!descriptionsMap.get(obj.id);
      const tier = classifyTier(obj, hasRetrievableDesc);
      totalWciSum += computeWCI(obj, hasRetrievableDesc);
  ...
  const enrichmentStats = {
    ...
    total_retrievable_description: totalRetrievableDesc,
    retrievable_description_pct: totalEnriched > 0 ? Math.round(100 * totalRetrievableDesc / totalEnriched) : 0,
    silent_rot_count: silentRotCount,
    weighted_completeness_index: totalEnriched > 0 ? Math.round(totalWciSum / totalEnriched) : 0,
  ...
  // ENR-PIPELINE-RUNTIME-1: emit start/end timestamps for DASH LatencyJourney cumulative-E2E.
  enrichmentStats.enrichment_started_at = ctx.enrichmentStartedAt || null;
  enrichmentStats.enrichment_completed_at = new Date().toISOString();
  // ENR-SIDECAR-READAT-1: when ENR finished reading sidecars. Compare to AGG's jobs-metadata.json
  // latency_markers.sidecars_written_at — if sidecar_read_at < sidecars_written_at, ENR read a stale
  // version (AGG updated after ENR's read). Lets check-32/operator distinguish real-loss from lag.
  enrichmentStats.sidecar_read_at = ctx.sidecarReadAt || null;
  // ENR-SIDECAR-INTEGRITY-CHECK-1: per-source sidecar entry quality (detects AGG writing empty descriptions).
  // Distinguishes "missing sidecar entry" (check-23 coverage) from "empty-write entry" (AGG bug class).
  // Sample that motivated this: 5/5 t0 workday jobs (BorgWarner/Semtech/etc.) had sidecar entries
  // with description_text length=0 — AGG wrote the entry but with empty content.
  const sidecarStats = ctx.sidecarStats || {};
  const sidecarIntegrity = {};
  for (const [src, s] of Object.entries(sidecarStats)) {
    if (s.total === 0) continue;
    sidecarIntegrity[src] = {
      total: s.total,
      empty: s.empty,
      empty_pct: Math.round((s.empty / s.total) * 1000) / 10,  // 1 decimal place
    };
  }
  enrichmentStats.sidecar_integrity = sidecarIntegrity;
  // ENR-DESCCACHE-OBSERVABILITY-1: surface failCache (silent description-fetch failures) for observability.
  // Without this, failCache is invisible — Snap-class bugs (jobs failCaching permanently with no alarm)
  // go undetected for months. Surfaces total count + per-source breakdown (top sources by fail count).
  const failCache = ctx.failCache || {};
  const failCacheBySource = {};
  const allJobsById = new Map(allJobs.map(j => [j.id, j]));
  for (const id of Object.keys(failCache)) {
    const job = allJobsById.get(id);
    const src = job?.source || 'unknown';
    failCacheBySource[src] = (failCacheBySource[src] || 0) + 1;
  }
  enrichmentStats.fail_cache = {
    total: Object.keys(failCache).length,
    by_source: Object.entries(failCacheBySource)
      .sort((a, b) => b[1] - a[1])
      .reduce((acc, [src, count]) => { acc[src] = count; return acc; }, {}),
  };
  ...
  const techUsTruth = {
    ...
    numerator: {
      ...
      enriched_retrievable_description: totalRetrievableDesc,
...
module.exports = { generateStats, classifyTier, computeWCI, TECH_DOMAINS };
