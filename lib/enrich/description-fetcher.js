...
const STALE_STRUCTURED_REFRESH_PER_RUN = 100; // Gradually rewrite legacy flat WD/SR sidecars into dual-text structured entries.
...
async function refreshStaleSrDescriptions(allJobs, descriptionsMap, enrichedIds, flatIds, activeChunkPath) {
  ...
  const batch = stale.slice(0, STALE_STRUCTURED_REFRESH_PER_RUN);
