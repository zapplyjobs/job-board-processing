...
function buildWdDescUrl(jobUrl) {
  ...
  if (site) {
    ...
    const tenant = MYWORKDAYSITE_TENANTS[site[2]];
    if (!tenant) {
      // ENR-SOURCE-METADATA-ARCH-1 (2026-07-19): no architectural change yet (only Snap known).
      // Warn so a new myworkdaysite.com tenant surfacing in the pool is visible in enrich-jobs logs
      // + gets routed to failCache observability — operator adds it to MYWORKDAYSITE_TENANTS.
      console.warn(`[enrich-jobs] UNKNOWN myworkdaysite.com tenant: career slug '${site[2]}' not in MYWORKDAYSITE_TENANTS map — will likely 422 + failCache. Add to map if tenant name differs.`);
    }
    const resolvedTenant = tenant || site[2];
    return `${site[1]}/wday/cxs/${resolvedTenant}/${site[2]}${site[3]}`;
  }
  return null;
}
...
function loadDescriptionsMap() {
  ...
  // ENR-SIDECAR-INTEGRITY-CHECK-1: track per-source sidecar entry quality.
  // Detects AGG writing entries with empty description_text (root cause of current production
  // alarms — sample: 5/5 t0 workday jobs BorgWarner/Semtech/etc. have sidecar entries with
  // desc len 0). Source derived from filename (descriptions-{source}(-N)?.jsonl).
  const sidecarStats = {};
  ...
  for (const filePath of files) {
    ...
    // ENR-SIDECAR-INTEGRITY-CHECK-1: derive source from filename for per-source stats.
    const sourceMatch = path.basename(filePath).match(/^descriptions-([a-z]+)(-\d+|-enriched-\d+)?\.jsonl$/);
    const source = sourceMatch ? sourceMatch[1] : 'unknown';
    if (!sidecarStats[source]) sidecarStats[source] = { total: 0, empty: 0 };
    const lines = fs.readFileSync(filePath, 'utf8').trim().split('\n').filter(Boolean);
    for (const line of lines) {
      try {
        const { id, description_text, extraction_text, fetched_at } = JSON.parse(line);
        if (id) {
          sidecarStats[source].total++;
          const text = extraction_text || description_text || null;
          if (!text || text.length === 0) sidecarStats[source].empty++;  // ENR-SIDECAR-INTEGRITY-CHECK-1
          map.set(id, text);
  ...
  return { map, enrichedIds, flatIds, staleByAgeIds, sidecarStats };
}
