...
const STALE_STRUCTURED_REFRESH_PER_RUN = 250; // Gradually rewrite legacy flat WD/SR sidecars into dual-text structured entries while staying inside the normal ENR runtime target.
...
async function refreshStaleSrDescriptions(allJobs, descriptionsMap, enrichedIds, flatIds, activeChunkPath, currentEnrichedById = new Map()) {
  ...
  for (const job of batch) {
    if (Date.now() - startTime > 75 * 1000) break; // 75s cap for stale refresh — keeps migration moving while pulling runtime back under the normal <4 min target
