...
const STALE_STRUCTURED_REFRESH_PER_RUN = 200; // Gradually rewrite legacy flat WD/SR sidecars into dual-text structured entries.
...
function prioritizeStructuredRefreshCandidates(stale, limit = STALE_STRUCTURED_REFRESH_PER_RUN) {
  return [...stale]
    .sort((a, b) => String(b.posted_at || '').localeCompare(String(a.posted_at || '')))
    .slice(0, limit);
}

async function refreshStaleSrDescriptions(allJobs, descriptionsMap, enrichedIds, flatIds, activeChunkPath) {
  const failCache = loadFailCache();
  ...
  const batch = prioritizeStructuredRefreshCandidates(stale);
