...
class PostedJobsManagerV2 {
  ...
  markAsPosted(jobId, jobData, discordThreadId = null) {
    ...
    const newJob = {
      ...
      source: jobData.source || null,  // ATS source (workday/greenhouse/ashby/etc.) from all_jobs.json — for DASH display (OUT-POST-1)
   ...
   */
  markAsPostedToChannel(jobData, messageId, channelId, channelType, channelJobNumber = null, channelName = null) {
    const now = new Date().toISOString();
    ...
    jobRecord.discordPosts[channelId] = {
      ...
      // OUT-CHANNEL-NAMES-1: persist channelName (already fetched by discord-poster.js via channels.fetch).
      // Enables downstream aggregators (publish-metrics-to-supabase.js byChannel) to use human-readable names
      // instead of snowflake IDs. Backward-compat: null for legacy posts; aggregators fall back to channelId.
