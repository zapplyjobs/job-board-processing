...
function addIdsFromRecords(records, activeIds) {
...
function addIdsFromJsonOrNdjsonFile(filePath, activeIds) {
  ...
  if (raw[0] === '[' || raw[0] === '{') {
    try {
      ...
      if (Array.isArray(parsed)) return addIdsFromRecords(parsed, activeIds);
      ...
      // Fall through to NDJSON parsing. Some JSONL files start with `{`.
    }
  }
...
addIdsFromJsonOrNdjsonFile(allJobsPath, activeIds);
...
if (fs.existsSync(processedPath)) {
  try {
    addIdsFromJsonOrNdjsonFile(processedPath, activeIds);
...
if (fs.existsSync(enrichedPath)) {
  ...
  addIdsFromJsonOrNdjsonFile(enrichedPath, activeIds);
  const enrichedCount = activeIds.size - before;
  if (enrichedCount > 0) {
...
for (const filename of descFiles) {
  ...
  for (let i = 0; i < fileLines.length; i++) {
    ...
    try {
      ...
      if (activeIds.has(entry.id)) {
        latestById.set(entry.id, line); // last active entry wins
      } else {
        removed++;
      }
    } catch {
      ...
      latestById.set(`__malformed_${i}`, line);
    }
  }

  const entries = [...latestById.values()];
  kept = entries.length - malformed;
  ...
  if (!dryRun && (removed > 0 || entries.length !== fileLines.filter(Boolean).length)) {
    fs.writeFileSync(filePath, entries.join('\n') + '\n', 'utf8');
  }
}

// Step 5: Rebuild enriched chunks so the latest active entry wins across the whole chunk set.
// This prevents old flat and newer structured entries for the same id from coexisting forever.
const enrichedChunks = descFiles.filter(f => /^descriptions-enriched-\d+\.jsonl$/.test(f));
if (enrichedChunks.length > 0 && !dryRun) {
  const latestById = new Map();
  for (const chunk of enrichedChunks) {
    const chunkPath = path.join(DATA_DIR, chunk);
    const lines = fs.readFileSync(chunkPath, 'utf8').trim().split('\n').filter(Boolean);
    for (const line of lines) {
      try {
        const entry = JSON.parse(line);
        if (entry.id) latestById.set(entry.id, line);
      } catch {
        latestById.set(`__malformed_${chunk}_${latestById.size}`, line);
      }
    }
  }

  const dedupedLines = [...latestById.values()];
  ...
  for (const line of dedupedLines) {
    ...
    if (current.length > 0 && currentBytes + lineBytes > CHUNK_LIMIT_BYTES) {
      rebuiltChunks.push(current);
      current = [];
      currentBytes = 0;
    }
    current.push(line);
  ...
  if (current.length > 0) rebuiltChunks.push(current);

  for (const chunk of enrichedChunks) fs.unlinkSync(path.join(DATA_DIR, chunk));
  rebuiltChunks.forEach((lines, idx) => {
    const file = path.join(DATA_DIR, `descriptions-enriched-${idx + 1}.jsonl`);
    fs.writeFileSync(file, lines.join('\n') + '\n', 'utf8');
  });
  console.log(`[prune-sidecars] Rebuilt ${rebuiltChunks.length} enriched chunk(s) from ${enrichedChunks.length} after global latest-entry dedupe`);
}
