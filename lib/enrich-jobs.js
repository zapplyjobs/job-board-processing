...
// Config
// ---------------------------------------------------------------------------
const ENRICHER_VERSION = 75;   // VISA-CLEARANCE: capture security-clearance eligibility language in unresolved defense/regulated postings
...
const latest = latestEnrichedMetaById.get(job.id) || { version: 0, hasDescription: false };
...
const reservedStale = [...staleNoDesc, ...staleOther].slice(0, Math.min(staleReserve, fastBatchSize));
...
return { fastBatch: [...reservedStale, ...remainingFresh], staleReserved: reservedStale.length, noDescReserved };
}
...
if (fs.existsSync(ENRICHED_PATH)) {
  ...
  for (let i = enrichedLines.length - 1; i >= 0; i--) {
    try {
      ...
      latestEnrichedMetaById.set(obj.id, { version: latestVersion, hasDescription: !!obj.has_description });
...
console.log(`[enrich-jobs] Processing batch: ${fastBatch.length} fast + ${slowBatch.length} slow = ${batch.length} total${staleReserved > 0 ? ` (includes ${staleReserved} stale fast records${noDescReserved > 0 ? `, ${noDescReserved} no-desc` : ''})` : ''}`);
