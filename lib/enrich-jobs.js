...
const STALE_FAST_RESERVE = 100; // Reserve enough fast-batch capacity for newly shipped field fixes to surface within a few cycles instead of dozens.
...
function buildFastBatch(fastPending, latestEnrichedMetaById, currentVersion, fastBatchSize = FAST_BATCH_SIZE, staleReserve = STALE_FAST_RESERVE) {
const staleNoDesc = [];
const staleNoSkills = [];
const staleNoVisa = [];
const staleOther = [];
...
const reserveLimit = Math.min(staleReserve, fastBatchSize);
const reservedStale = [];
...
return { fastBatch: [...reservedStale, ...remainingFresh], staleReserved: reservedStale.length, noDescReserved, noSkillsReserved, noVisaReserved };
}
...
const { fastBatch, staleReserved, noDescReserved, noSkillsReserved, noVisaReserved } = buildFastBatch(fastPending, latestEnrichedMetaById, ENRICHER_VERSION);
const slowBatch = slowPending.slice(0, SLOW_BATCH_SIZE);
const batch = [...fastBatch, ...slowBatch];
console.log(`[enrich-jobs] Processing batch: ${fastBatch.length} fast + ${slowBatch.length} slow = ${batch.length} total${staleReserved > 0 ? ` (includes ${staleReserved} stale fast records${noDescReserved > 0 ? `, ${noDescReserved} no-desc` : ''}${noSkillsReserved > 0 ? `, ${noSkillsReserved} no-skills` : ''}${noVisaReserved > 0 ? `, ${noVisaReserved} no-visa` : ''})` : ''}`);
