...
function buildFastBatch(fastPending, latestEnrichedVersionById, currentVersion, fastBatchSize = FAST_BATCH_SIZE, staleReserve = STALE_FAST_RESERVE) {
const stale = [];
const fresh = [];
for (const job of fastPending) {
const latestVersion = latestEnrichedVersionById.get(job.id) || 0;
if (latestVersion > 0 && latestVersion < currentVersion) stale.push(job);
else fresh.push(job);
}
const reservedStale = stale.slice(0, Math.min(staleReserve, fastBatchSize));
...
return { fastBatch: [...reservedStale, ...remainingFresh], staleReserved: reservedStale.length };
}
...
const latestEnrichedVersionById = new Map();
if (fs.existsSync(ENRICHED_PATH)) {
  ...
  for (let i = enrichedLines.length - 1; i >= 0; i--) {
    try {
      ...
      latestEnrichedVersionById.set(obj.id, latestVersion);
...
const { fastBatch, staleReserved } = buildFastBatch(fastPending, latestEnrichedVersionById, ENRICHER_VERSION);
const slowBatch = slowPending.slice(0, SLOW_BATCH_SIZE);
const batch = [...fastBatch, ...slowBatch];
console.log(`[enrich-jobs] Processing batch: ${fastBatch.length} fast + ${slowBatch.length} slow = ${batch.length} total${staleReserved > 0 ? ` (includes ${staleReserved} stale fast records)` : ''}`);
