...
const { loadDescriptionsMap, resolveActiveChunk, fetchMissingDescriptions, refreshStaleSrDescriptions, refreshStaleContentDescriptions, buildWdDescUrl, buildSrDescUrl, quickGet, loadFailCache, saveFailCache, syncFailCacheFromR2, syncFailCacheToR2 } = require('./enrich/description-fetcher');
const { generateStats, classifyTier, TECH_DOMAINS } = require('./enrich/stats');

// ---------------------------------------------------------------------------
// Config
// ---------------------------------------------------------------------------
const ENRICHER_VERSION = 89;   // ENR-D4 taxonomy (2026-06-21): +4 engineering terms (3-phase, power distribution, IEC 61010, IEC 62368) — recovers 8 electrical/power/safety-engineering rows the software-focused taxonomy missed; 0 regressions, 0 false positives (procedure Phases 1-4 passed). v88 was ENR-C181 D1 description freshness.
...
const STALE_FAST_RESERVE = 100; // Minimum stale fast capacity; adaptive logic raises this when version propagation dominates.
const MIN_FAST_FRESH_SLOTS = 100; // Protect fresh fast jobs while stale records drain after a version bump.
...
function calculateAdaptiveStaleReserve(staleCount, freshCount, fastBatchSize = FAST_BATCH_SIZE, baseReserve = STALE_FAST_RESERVE, minFreshSlots = MIN_FAST_FRESH_SLOTS) {
const batchSize = Math.max(0, Number(fastBatchSize) || 0);
const stale = Math.max(0, Number(staleCount) || 0);
const fresh = Math.max(0, Number(freshCount) || 0);
if (batchSize === 0 || stale === 0) return 0;
if (fresh === 0) return Math.min(stale, batchSize);

const base = Math.min(Math.max(0, Number(baseReserve) || 0), batchSize);
const freshFloor = Math.min(Math.max(0, Number(minFreshSlots) || 0), Math.floor(batchSize * 0.2));
const protectedFresh = Math.min(fresh, freshFloor, batchSize);
const maxReserve = Math.max(0, batchSize - protectedFresh);
const staleRatio = stale / (stale + fresh);

let target = base;
if (staleRatio >= 0.5) target = Math.max(target, Math.ceil(batchSize * 0.8));
else if (staleRatio >= 0.25) target = Math.max(target, Math.ceil(batchSize * 0.6));
else if (staleRatio >= 0.1) target = Math.max(target, Math.ceil(batchSize * 0.4));

return Math.min(stale, maxReserve, target);
}

function buildFastBatch(fastPending, latestEnrichedMetaById, currentVersion, fastBatchSize = FAST_BATCH_SIZE, staleReserve = null) {
const staleNoDesc = [];
const staleNoSkills = [];
const staleNoVisa = [];
const staleOther = [];
const fresh = [];
for (const job of fastPending) {
const latest = latestEnrichedMetaById.get(job.id) || { version: 0, hasDescription: false, hasSkills: false, hasVisa: false };
if (latest.version > 0 && latest.version < currentVersion) {
if (latest.hasDescription === false) staleNoDesc.push(job);
else if (latest.hasSkills === false) staleNoSkills.push(job);
else if (latest.hasVisa === false) staleNoVisa.push(job);
else staleOther.push(job);
} else {
fresh.push(job);
}
}
const staleCount = staleNoDesc.length + staleNoSkills.length + staleNoVisa.length + staleOther.length;
const reserveLimit = staleReserve === null || staleReserve === undefined
? calculateAdaptiveStaleReserve(staleCount, fresh.length, fastBatchSize)
: Math.min(Math.max(0, staleReserve), fastBatchSize);
const reservedStale = [];
const reservedIds = new Set();
const addFromBucket = (bucket, limit) => {
for (const job of bucket) {
if (reservedStale.length >= reserveLimit || limit <= 0) break;
if (reservedIds.has(job.id)) continue;
reservedStale.push(job);
reservedIds.add(job.id);
limit--;
}
};
// Keep one stale class from monopolizing the reserve. New field fixes often target
// visa or degree while no-skill residuals are larger; a single priority concat can
// delay the just-shipped fix for dozens of cycles.
const baseQuota = reserveLimit >= 3 ? Math.floor(reserveLimit / 3) : 0;
if (baseQuota > 0) {
addFromBucket(staleNoDesc, baseQuota);
addFromBucket(staleNoSkills, baseQuota);
addFromBucket(staleNoVisa, baseQuota);
}
addFromBucket(staleNoDesc, reserveLimit);
addFromBucket(staleNoSkills, reserveLimit);
addFromBucket(staleNoVisa, reserveLimit);
addFromBucket(staleOther, reserveLimit);
const staleIds = new Set(reservedStale.map(j => j.id));
const remainingFresh = fresh.filter(j => !staleIds.has(j.id)).slice(0, Math.max(0, fastBatchSize - reservedStale.length));
const noDescReserved = reservedStale.filter(j => staleNoDesc.some(s => s.id === j.id)).length;
const noSkillsReserved = reservedStale.filter(j => staleNoSkills.some(s => s.id === j.id)).length;
const noVisaReserved = reservedStale.filter(j => staleNoVisa.some(s => s.id === j.id)).length;
return { fastBatch: [...reservedStale, ...remainingFresh], staleReserved: reservedStale.length, staleReserveLimit: reserveLimit, noDescReserved, noSkillsReserved, noVisaReserved };
}

function buildVisaDetectionText(plainText, pageDescription) {
const pageText = toPlainText(pageDescription || '');
if (!pageText || pageText === plainText || plainText.includes(pageText)) return plainText;
return `${plainText}\n${pageText}`;
}
...
const { visaPresent: visaQuestionPresent, questionCount, pageDescription } = await fetchApplicationVisaStatus(job);
const rawDescription = descriptionsMap.get(job.id) || job.description || pageDescription || null;
...
const sponsorsVisa = detectVisa(buildVisaDetectionText(plainText, pageDescription));
...
url: job.url || null,
posted_at: job.posted_at || job.fetched_at || null,
};
}

async function main() {
console.log('[enrich-jobs] Starting enrichment run');
// ENR-DESCCACHE-OBSERVABILITY-1: sync failCache from R2 so fresh runners inherit prior failures
// (instead of re-attempting known-bad URLs) + so failCache persists across runs for observability.
await syncFailCacheFromR2();
// ENR-PIPELINE-RUNTIME-1: capture script start for DASH LatencyJourney cumulative-E2E.
const ENRICHMENT_STARTED_AT = new Date().toISOString();

const termMap = loadTaxonomy();
console.log(`[enrich-jobs] Taxonomy loaded: ${termMap.size} terms`);


const { map: descriptionsMap, enrichedIds: descEnrichedIds, flatIds: descFlatIds, staleByAgeIds: descStaleByAgeIds, sidecarStats: SIDECAR_STATS } = loadDescriptionsMap();
// ENR-SIDECAR-READAT-1: capture when ENR finished reading sidecars. Compare against AGG's
// jobs-metadata.json latency_markers.sidecars_written_at to detect stats-lag (ENR read stale
// version written before AGG's latest update). Lets check-32 distinguish real-loss from timing.
const SIDECAR_READ_AT = new Date().toISOString();
console.log(`[enrich-jobs] Descriptions loaded: ${descriptionsMap.size} entries (${descEnrichedIds.size} from enriched sidecars, ${descFlatIds.size} legacy flat entries, ${descStaleByAgeIds.size} stale-by-age) | sidecar_read_at: ${SIDECAR_READ_AT}`);
...
// ENR-DESCRETRIEVE-1: load the pre-fetch enriched-id set once (enriched_jobs.json / processed_ids.json
// are stable mid-run) — used for recovery-first fetch prioritization below and reused at the
// stale-refresh + skip/rescue stages instead of re-reading the same files.
const preFetchEnriched = loadEnrichedIds();
await fetchMissingDescriptions(allJobs, descriptionsMap, activeChunkPath, new Set(preFetchEnriched.latestById.keys()));

// DESC-STRUCT: Gradually rewrite old flat WD/SR sidecars into dual-text structured entries.
const currentEnrichedById = preFetchEnriched.latestById;
await refreshStaleSrDescriptions(allJobs, descriptionsMap, descEnrichedIds, descFlatIds, activeChunkPath, currentEnrichedById);
// D1 (C181): Re-fetch WD/SR on-demand descriptions whose stored copy may be content-stale
// (posting changed since fetch — e.g. export-control/eligibility language added later). Bounded catch-up.
await refreshStaleContentDescriptions(allJobs, descriptionsMap, descStaleByAgeIds, activeChunkPath, currentEnrichedById);

const processedMap = loadProcessedMap();
// ENR-DONECHECK-CONSOLIDATION-1: QueueState centralizes all done-check decisions + mutations
const queue = new QueueState({
  processedMap,
  enrichedJobsById: preFetchEnriched.latestById,
  descriptionsMap,
  currentVersion: ENRICHER_VERSION,
  techDomains: TECH_DOMAINS,
  structuralSources: STRUCTURAL_SOURCES,
  maxRetries: MAX_RETRIES,
});

let resurrectedSkipped = 0;
for (const job of allJobs) {
  if (queue.shouldResurrect(job)) {
    queue.resurrect(job.id);
    resurrectedSkipped++;
  }
}
if (resurrectedSkipped > 0) {
  console.log(`[enrich-jobs] Resurrected ${resurrectedSkipped} skipped records that are now enrichable`);
}

const { latestById } = preFetchEnriched;
// ENR-SOURCE-2: Current-version exhausted records can become enrichable later when
// a sidecar description arrives after they were exhausted. Rescue them back into the queue.
let rescuedExhausted = 0;
for (const job of allJobs) {
  if (queue.shouldRescue(job)) {
    queue.rescue(job.id);
    rescuedExhausted++;
  }
}
...
const pending = allJobs.filter(j => !queue.isDone(j.id));
console.log(`[enrich-jobs] Already enriched: ${allJobs.length - pending.length}`);
...
if (queue.structuralSources.has(job.source) && !queue.processedMap[job.id]) {
queue.markSkipped(job.id, 'structural_no_desc', now);
bulkMarked++;
}
}
if (bulkMarked > 0) {
...
const enrichablePending = pending.filter(j => queue.isEnrichable(j));
...
const latestEnrichedMetaById = new Map();
if (fs.existsSync(ENRICHED_PATH)) {
  ...
  for (let i = enrichedLines.length - 1; i >= 0; i--) {
    try {
      ...
      seen.add(obj.id);
      const latestVersion = obj.enricher_version || 0;
      latestEnrichedMetaById.set(obj.id, { version: latestVersion, hasDescription: !!obj.has_description, hasSkills: Array.isArray(obj.required_skills) && obj.required_skills.length > 0, hasVisa: (obj.sponsors_visa !== null && obj.sponsors_visa !== undefined) || (obj.possible_sponsor !== null && obj.possible_sponsor !== undefined) || (obj.visa_question_present !== null && obj.visa_question_present !== undefined) });
      if (pendingIds.has(obj.id) && latestVersion < ENRICHER_VERSION) {
        reenrichmentPending++;
      }
    } catch (_) {}
  }
...
const { fastBatch, staleReserved, staleReserveLimit, noDescReserved, noSkillsReserved, noVisaReserved } = buildFastBatch(fastPending, latestEnrichedMetaById, ENRICHER_VERSION);
const slowBatch = slowPending.slice(0, SLOW_BATCH_SIZE);
const batch = [...fastBatch, ...slowBatch];
console.log(`[enrich-jobs] Processing batch: ${fastBatch.length} fast + ${slowBatch.length} slow = ${batch.length} total${staleReserved > 0 ? ` (includes ${staleReserved} stale fast records from adaptive reserve ${staleReserveLimit}${noDescReserved > 0 ? `, ${noDescReserved} no-desc` : ''}${noSkillsReserved > 0 ? `, ${noSkillsReserved} no-skills` : ''}${noVisaReserved > 0 ? `, ${noVisaReserved} no-visa` : ''})` : ''}`);
...
const liveIds = new Set(allJobs.map(j => j.id));
// ENR-POSTEDAT-PROP: posted_at is denormalized from all_jobs. Already-enriched records freeze
// their posted_at at enrich time, so a corrected all_jobs value never reaches them unless we
// reconcile here. Fill-null-only (never overwrite a valid date — operator constraint).
const allJobsPostedAtById = new Map();
for (const j of allJobs) {
  const pa = j.posted_at || j.fetched_at;
  if (j.id && pa) allJobsPostedAtById.set(j.id, pa);
}
let postedAtReconciled = 0;
...
if (hasResults) {
queue.markEnriched(job.id, now);
} else {
const retryCount = queue.computeRetryCount(job.id);
if (retryCount >= MAX_RETRIES) {
queue.markExhausted(job.id, retryCount, now);
exhaustedMarked++;
} else {
queue.markRetry(job.id, retryCount, now);
retryMarked++;
}
}
}
...
// ENR-POSTEDAT-PROP: backfill null posted_at from the live all_jobs pool (source of truth).
let lineOut = allEnrichedLines[i];
if (!obj.posted_at && allJobsPostedAtById.has(obj.id)) {
  obj.posted_at = allJobsPostedAtById.get(obj.id);
  lineOut = JSON.stringify(obj);
  postedAtReconciled++;
}
prunedLines.push(lineOut);
} catch (_) {
...
if (removed > 0 || postedAtReconciled > 0) {
fs.writeFileSync(ENRICHED_PATH, finalLines.join('\n') + '\n', 'utf8');
console.log(`[enrich-jobs] Pruned ${removed} records (expired + deduped)${postedAtReconciled > 0 ? ` + reconciled ${postedAtReconciled} null posted_at from all_jobs` : ''}`);
}
...
// ENR-DESCCACHE-OBSERVABILITY-1: surface failCache stats (size + per-source breakdown) for observability.
const failCacheForStats = loadFailCache();
const enrichmentStats = generateStats({ allJobs, finalLines, processedMap, descriptionsMap, DATA_DIR, ENRICHER_VERSION, descWaiting, reenrichmentPending, enrichmentStartedAt: ENRICHMENT_STARTED_AT, failCache: failCacheForStats, sidecarReadAt: SIDECAR_READ_AT, sidecarStats: SIDECAR_STATS });
// Upload failCache to R2 so monitoring + next run can see it.
await syncFailCacheToR2();
}
}
...
buildWdDescUrl,
buildSrDescUrl,
calculateAdaptiveStaleReserve,
buildFastBatch,
buildVisaDetectionText,
ENRICHER_VERSION,
};
