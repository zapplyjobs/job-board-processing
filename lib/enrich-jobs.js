...
// Config
// ---------------------------------------------------------------------------
const ENRICHER_VERSION = 61;   // ENR-SR-QUAL-1: SR description fetcher now includes qualifications section (skills + degree)
...
// Enriched sidecars last — their qualification-rich descriptions override
// source sidecar boilerplate (last-write-wins in map.set).
.sort((a, b) => {
...
try {
const data = JSON.parse(result.body);
// ENR-SR-QUAL-1: SR API has separate sections — jobDescription (role) and qualifications (skills/degree).
              // Concatenating both ensures the enricher sees skill terms and degree requirements.
...
// Pre-catch-all: specific roles that don't contain "engineer"/"developer"/"analyst"
...
else if (hasDegree && hasVisa) tier = 4;
else if (hasDegree) tier = 3;
else tier = 2; // has skills but missing degree
...
if (tier >= 3) companyMap[co].t3_plus = (companyMap[co].t3_plus || 0) + 1; // T3+T4 combined
} catch (_) {}
}
...
// Stuck-record detection: Google/Apple records with qualification-rich descriptions
// in enriched sidecars but still below T3 (version not reset, never re-processed).
...
for (const [id, desc] of descriptionsMap) {
  ...
  if (rec && rec.enricher_version > 0) {
    const hasSkills = rec.required_skills?.length > 0;
    const hasDegree = rec.min_degree != null;
    const hasVisa = rec.sponsors_visa != null || rec.visa_question_present != null || rec.possible_sponsor != null;
    const tier = !rec.has_description ? 0 : !hasSkills ? 1 : (hasDegree && hasVisa) ? 4 : hasDegree ? 3 : 2;
