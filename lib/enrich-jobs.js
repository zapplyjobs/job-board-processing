...
// Config
// ---------------------------------------------------------------------------
const ENRICHER_VERSION = 76;   // VISA-CURRENT-NEGATIVES: reprocess repeated current-version negative visa phrase families
...
const latest = latestEnrichedMetaById.get(job.id) || { version: 0, hasDescription: false, hasVisa: false };
...
const reservedStale = [...staleNoDesc, ...staleNoVisa, ...staleOther].slice(0, Math.min(staleReserve, fastBatchSize));
...
return { fastBatch: [...reservedStale, ...remainingFresh], staleReserved: reservedStale.length, noDescReserved, noVisaReserved };
}
...
if (fs.existsSync(ENRICHED_PATH)) {
  ...
  for (let i = enrichedLines.length - 1; i >= 0; i--) {
    try {
      ...
      latestEnrichedMetaById.set(obj.id, { version: latestVersion, hasDescription: !!obj.has_description, hasVisa: (obj.sponsors_visa !== null && obj.sponsors_visa !== undefined) || (obj.possible_sponsor !== null && obj.possible_sponsor !== undefined) || (obj.visa_question_present !== null && obj.visa_question_present !== undefined) });
...
const { fastBatch, staleReserved, noDescReserved, noVisaReserved } = buildFastBatch(fastPending, latestEnrichedMetaById, ENRICHER_VERSION);
const slowBatch = slowPending.slice(0, SLOW_BATCH_SIZE);
const batch = [...fastBatch, ...slowBatch];
console.log(`[enrich-jobs] Processing batch: ${fastBatch.length} fast + ${slowBatch.length} slow = ${batch.length} total${staleReserved > 0 ? ` (includes ${staleReserved} stale fast records${noDescReserved > 0 ? `, ${noDescReserved} no-desc` : ''}${noVisaReserved > 0 ? `, ${noVisaReserved} no-visa` : ''})` : ''}`);
