...
if (STRUCTURAL_SOURCES.has(job.source)) return false;
...
function shouldResurrectSkippedRecord(job, processedEntry, descriptionsMap) {
if (processedEntry?.status !== 'skipped') return false;
if (!['non-tech', 'non-us'].includes(processedEntry.reason)) return false;
return isEnrichable(job, descriptionsMap);
}
