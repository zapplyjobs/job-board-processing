...
const CONSUMER_REPOS = [
  ...
  { repo: 'New-Grad-Healthcare-Jobs-2027',               label: 'Nursing' },
];
...
async function main() {
  ...
  if (fs.existsSync(allJobsPath)) {
    try {
      ...
      const parts = ['workday','greenhouse','amazon','ashby','lever','simplify']
        .filter(k => bySource[k])
  ...
  // Count comes from current_jobs.json when present, but freshness must reflect the
  // latest consumer update cycle, not the last time current_jobs.json changed. Some
  // repos commit README.md + last_count.json every run while current_jobs.json only
  // changes when the payload changes, so using path-scoped commit history can report
  // wildly stale ages even when the repo is updating on schedule.
  pipelineLines += '\n';
  ...
  const trafficRepos = ['New-Grad-Jobs-2027','Internships-2027','New-Grad-Software-Engineering-Jobs-2027',
    'New-Grad-Data-Science-Jobs-2027','New-Grad-Hardware-Engineering-Jobs-2027','New-Grad-Healthcare-Jobs-2027',
