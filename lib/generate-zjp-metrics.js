...
// INF-STALE-METRICS-FIELDS-1: Consumer repos removed — they're pure display now
// (pipeline pushes READMEs via Contents API, no submodules). Only aggregator/processing
// repos still carry submodules.
const SUBMODULE_CONFIG = {
  shared: {
    source_repo: 'zapplyjobs/job-board-shared',
    repos: ['jobs-aggregator-private', 'jobs-data-2026'],
  },
  aggregator: {
...
async function getPipelineStatus(enrichStats) {
  const status = {
    ...
    // INF-ENR-RUNTIME-COLLECT-1: ENR-PIPELINE-RUNTIME-1 producer emits these to enrichment-stats.json.
    // Exposed here so DASH LatencyJourney can read enrichment duration via the same pipeline.* path
    // it already uses for aggregator_runtime_minutes (DASH-LATENCY-CUMULATIVE-1 consumer).
    enrichment_started_at: null,
    enrichment_completed_at: null,
    enrichment_runtime_minutes: null,
  };
  ...
  // INF-ENR-RUNTIME-COLLECT-1: pull script-internal timestamps from enrichment-stats.json (producer
  // shipped in commit a7a13b0). These are NOT the GH Actions workflow timestamps above — they're
  // the actual script start/end, so runtime is real execution time (not GH queue + execution).
  if (enrichStats?.enrichment_started_at && enrichStats?.enrichment_completed_at) {
    status.enrichment_started_at = enrichStats.enrichment_started_at;
    status.enrichment_completed_at = enrichStats.enrichment_completed_at;
    const dur = (new Date(enrichStats.enrichment_completed_at) - new Date(enrichStats.enrichment_started_at)) / 60000;
    // Same guard as aggregator_runtime_minutes: sane window only
    if (dur > 0 && dur < 60) status.enrichment_runtime_minutes = Math.round(dur * 10) / 10;
  }
  ...
  for (const repo of CONSUMER_REPOS) {
    try {
      ...
      if (res.body?.[0]) {
        ...
        // INF-STALE-METRICS-FIELDS-1: positions removed — pipeline pushes READMEs, not job data
      ...
      consumers.repos[repo] = { last_commit: null };
    }
  }
...
async function getR2Health() {
  ...
  try {
    ...
    // Status flip tracks the ALERT threshold (thresholds.r2StaleMinutes, 60 min).
    // The WARN tier (warnings.r2StaleMinutes, 30 min) is handled by check-25.warn,
    // which fires when status is healthy but age >= the warn threshold. Both read
    // the SAME config so producer + alert layer can never desync.
    return {
      status: manifestAgeMin < config.thresholds.r2StaleMinutes ? 'healthy' : 'stale',
...
async function getDedupeStoreInfo() {
  try {
    // Read from Worker proxy (R2) — file was removed from git during R2 cutover (INF-DEDUPE-NOTFOUND-1)
    const res = await ghRequest('https://zjp-data-proxy.wild-queen-069e.workers.dev/data/dedupe-store.json');
    if (res.status !== 200) return { status: 'not_found', size_bytes: null, size_mb: null };
    const sizeBytes = Buffer.byteLength(JSON.stringify(res.body));
    return {
      status: 'tracked',
