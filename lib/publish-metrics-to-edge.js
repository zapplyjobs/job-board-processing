...
function summarizePostedJobs(data) {
  ...
  for (const j of jobs) {
    ...
    // OUT-CHANNEL-NAMES-1: prefer human-readable channelName (see publish-metrics-to-supabase.js for full chain).
    const firstPostId = j.discordPosts && Object.keys(j.discordPosts)[0];
    const firstPostName = firstPostId && j.discordPosts[firstPostId] && j.discordPosts[firstPostId].channelName;
