...
const SOURCES = [
  ...
  ['tag-history',      'tag-history.jsonl', summarizeTagHistory],
...
function summarizePostedJobs(data) {
  ...
  // OUT-CHANNEL-NAMES-1 fix: same dynamic channelName map as publish-metrics-to-supabase.js.
  const channelNameMap = {};
  for (const j of jobs) {
    if (j.discordPosts) {
      for (const [id, post] of Object.entries(j.discordPosts)) {
        if (post && post.channelName) channelNameMap[id] = post.channelName;
      }
    }
  }
  for (const j of jobs) {
    if (j.postedToDiscord && now - new Date(j.postedToDiscord).getTime() < dayMs) last24h++;
    const firstPostId = j.discordPosts && Object.keys(j.discordPosts)[0];
    const firstPostName = (firstPostId && j.discordPosts[firstPostId] && j.discordPosts[firstPostId].channelName) || (firstPostId && channelNameMap[firstPostId]);
    const ch = j.channel || j.channelName || firstPostName || firstPostId;
  ...
  // Zero-fill all 30 days for a complete series (INF-POSTEDJOBS-SUMMARY-FIX-1)
  const postsPerDay = [];
  for (let i = 29; i >= 0; i--) {
    const d = new Date(now - i * dayMs);
    const dayKey = d.toISOString().slice(0, 10);
    postsPerDay.push({ date: dayKey, count: dayBuckets[dayKey] || 0 });
  }
  return { total: jobs.length, last24h, byChannel, topReferrers, recentJobs, postsPerDay, summarizedAt: new Date().toISOString() };
}
// Custom summarizer for tag-history.jsonl (INF-LEVELDATA-1): transforms monitoring.js entries
// (G1/drift/precision schema) into the dashboard's TagHistoryEntry schema (date, g1_rate, levels).
// Deduplicates by day (keep latest per YYYY-MM-DD) to keep the array compact (~30 entries for 30d).
// Level fields (senior/mid_level/entry_level/internship) pass through as null until AGG adds
// them to monitoring.js entries — see advisory AGG-TAGHISTORY-LEVELS-1.
function summarizeTagHistory(data) {
  ...
  for (const e of arr) {
    ...
    byDay[day] = {
      date: ts,
      g1_rate: g1.us_general_rate_pct ?? null,
      us_general: g1.us_general ?? null,
      us_total: g1.us_total ?? null,
      senior: e.senior ?? null,
      mid_level: e.mid_level ?? null,
      entry_level: e.entry_level ?? null,
      internship: e.internship ?? null,
    };
  }
