...
async function main() {
  ...
  if (entries.length) {
    ...
    // Reset enricher_version for updated records so enrich-jobs re-processes them
    // with the new qualification text from detail pages.
    ...
    if (fs.existsSync(ePath)) {
      ...
      for (const line of lines) {
        try {
          ...
          if (updatedIds.has(obj.id) && (obj.enricher_version || 0) > 0) {
      ...
      console.log(`[detail-page-fetcher] Reset enricher_version for ${resetCount} records`);
    }
  }
