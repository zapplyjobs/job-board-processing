...
function needsDetailFetch(job, descs) {
  if (!(job?.source_id || job?.id)) return false;
  const desc = descs.get(job.id);
  return !desc || !hasQuals(desc);
}
...
function loadJobs(dir) {
  return parseJsonOrNdjsonRecords(fs.readFileSync(path.join(dir, 'all_jobs.json'), 'utf8'));
}
...
async function main() {
  ...
  const oj = jobs.filter(j => j.source === 'oracle');
  console.log(`[detail-page-fetcher] Google: ${gj.length}, Apple: ${aj.length}, Oracle: ${oj.length}`);

  const need = arr => arr.filter(j => needsDetailFetch(j, descs));
  const gNeed = need(gj), aNeed = need(aj);
  const oNeed = oj.filter(j => { const d = descs.get(j.id); return (!d || !hasQuals(d)) && j.url && j.source_id; });
  console.log(`[detail-page-fetcher] Need fetch: G=${gNeed.length} A=${aNeed.length} O=${oNeed.length}`);

  const t0 = Date.now(), entries = [];
  let gOk = 0, gFail = 0, aOk = 0, aFail = 0, oOk = 0, oFail = 0;
  ...
  // Oracle detail fetch — public REST API, no auth required
  if (oNeed.length) {
    const batch = oNeed.slice(0, ORACLE_BATCH);
    console.log(`[detail-page-fetcher] Oracle: ${batch.length} detail pages`);
    for (const j of batch) {
      if (Date.now()-t0 > MAX_RUNTIME_MS) { console.log('[detail-page-fetcher] Time limit'); break; }
      try {
        const urlObj = new URL(j.url);
        const baseUrl = `${urlObj.protocol}//${urlObj.host}`;
        const detailUrl = `${baseUrl}/hcmRestApi/resources/latest/recruitingCEJobRequisitionDetails?onlyData=true&finder=ById;Id=${j.source_id}`;
        const r = await httpGet(detailUrl);
        if (r) {
          const data = JSON.parse(r.body);
          const item = data?.items?.[0];
          if (item) {
            const parts = [];
            if (item.ExternalDescriptionStr) parts.push(stripHtml(item.ExternalDescriptionStr));
            if (item.ExternalResponsibilitiesStr) parts.push('Responsibilities:\n' + stripHtml(item.ExternalResponsibilitiesStr));
            if (item.ExternalQualificationsStr) parts.push('Qualifications:\n' + stripHtml(item.ExternalQualificationsStr));
            const newDesc = parts.join('\n\n');
            if (newDesc.length > (descs.get(j.id) || '').length) {
              entries.push({ id: j.id, description_text: newDesc });
              oOk++;
            } else oFail++;
          } else oFail++;
        } else oFail++;
      } catch { oFail++; }
      await new Promise(r => setTimeout(r, ORACLE_DELAY_MS));
    }
    console.log(`[detail-page-fetcher] Oracle: ${oOk}/${batch.length}`);
  }
  ...
  // Backlog drain: reset version for Google/Apple records that already have
  // qualification-rich descriptions in enriched sidecars but are stuck at v54.
  // The detail fetcher skips these (hasQuals=true) so they never get version-reset.
  const ePath = path.join(dataDir, 'enriched_jobs.json');
  if (fs.existsSync(ePath)) {
    const qualIds = new Set();
    for (const [id, d] of descs) {
      if ((id.startsWith('google-') || id.startsWith('apple-')) && hasQuals(d)) {
        qualIds.add(id);
      }
    }
    let resetCount = 0;
    const lines = fs.readFileSync(ePath, 'utf8').trim().split('\n');
    const out = [];
    for (const line of lines) {
      try {
        const obj = JSON.parse(line);
        // Only reset records at STALE versions (< 54). Records already at v54 with
        // no skills have content-absent descriptions — re-enrichment won't help.
        // Without this guard, 474 records entered an infinite loop:
        // reset v54→v0 → enrich at v54 → no skills → next run resets again.
        const hasSkills = (obj.required_skills?.length || 0) > 0;
        const v = obj.enricher_version || 0;
        if (qualIds.has(obj.id) && !hasSkills && v > 0 && v < 55) {
          obj.enricher_version = 0;
          resetCount++;
        }
        out.push(JSON.stringify(obj));
      } catch { out.push(line); }
    }
    fs.writeFileSync(ePath, out.join('\n') + '\n', 'utf8');
    if (resetCount) console.log(`[detail-page-fetcher] Backlog reset: ${resetCount} Google/Apple records queued for re-enrichment`);
  }

  console.log(`[detail-page-fetcher] Done ${((Date.now()-t0)/1000).toFixed(1)}s: ${gOk}G+${aOk}A+${oOk}O=${entries.length} total, remaining ${Math.max(0,gNeed.length-GOOGLE_BATCH)}G ${Math.max(0,aNeed.length-APPLE_BATCH)}A ${Math.max(0,oNeed.length-ORACLE_BATCH)}O`);
}
...
module.exports = { parseJsonOrNdjsonRecords, loadJobs, extractAppleQualifications, buildDesc, hasQuals, needsDetailFetch };